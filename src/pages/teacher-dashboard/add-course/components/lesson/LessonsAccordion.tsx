...
export function LessonsAccordion({ expanded, chapterIndex }: any) {
    ...
    function handleChangeVideo(video: Video) {
        setVideos(v => {
            let t = [...v];
            return t.map(n => n.id === video.id ? { ...video, id: n.id } : n)
        })
    }
    ...
    return (
        <Accordion
            ...
            <AccordionDetails sx={{
                ...
                {activeLesson === videos.length - 1 ?
                    <IconButton
                        onClick={() => {
                            setVideos((vids) => {
                                let v = [...vids]
                                v.push({ id: uuidv4(), title: '', description: '' })
                    ...
                    <IconButton
                        onClick={
                            () => setActiveLesson(l => l < videos.length - 1 ? (l + 1) : videos.length - 1)
                        }
                        sx={{
                            position: 'absolute',
                            left: '0%',
                ...
                <Typography color={'purple.main'} flexGrow={1}>
                    دروس الفصل
                </Typography>
                <Box sx={{
                    width: '100%',
